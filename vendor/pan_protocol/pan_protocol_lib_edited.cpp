 ...
 *      Original version.
 *   20-Feb-2022 (sun):
 *		- Add name to response message enum - PanguResponse
 *		- Prevent set_viewpoint_* and get_image methods from killing the
 *			program.
 *		- Make set_viewpoint_* and get_image methods return
 *PanguResponse enum
 *		- Add *&img argument to get_image as a means of acquiring the
 *image data pointer. {add_changes_here}
 *
 * Future work:
 *   - Change all methods to return bad PanguResponse instead of killing the
 *      program, returning data, being plain void.
 *	 - Add arguments for acquiring data from methods if applicable
 ...
 */
char *pan_net_safety_checks(void) {
  int s = 0;
  ...
  if ((s = (int)sizeof(unsigned char)) != 1) {
    (void)sprintf(tmp_buf, "Fatal: unexpected sizeof(unsigned char)=%d\n", s);
    assert(strlen(tmp_buf) + count < err_buf_size - 1);
    strcat(err_buf, tmp_buf);
    count += strlen(tmp_buf);
  }
  if ((s = (int)sizeof(unsigned short)) != 2) {
    (void)sprintf(tmp_buf, "Fatal: unexpected sizeof(unsigned short)=%d\n", s);
    assert(strlen(tmp_buf) + count < err_buf_size - 1);
    strcat(err_buf, tmp_buf);
    count += strlen(tmp_buf);
  }
  if ((s = (int)sizeof(float)) != 4) {
    (void)sprintf(tmp_buf, "Fatal: unexpected sizeof(float)=%d\n", s);
    assert(strlen(tmp_buf) + count < err_buf_size - 1);
    strcat(err_buf, tmp_buf);
    count += strlen(tmp_buf);
  }
  if ((s = (int)sizeof(double)) != 8) {
    (void)sprintf(tmp_buf, "Fatal: unexpected sizeof(double)=%d\n", s);
    assert(strlen(tmp_buf) + count < err_buf_size - 1);
    strcat(err_buf, tmp_buf);
    count += strlen(tmp_buf);
  }
  ...
  if ((s = (int)sizeof(unsigned long)) < 4) {
    (void)sprintf(tmp_buf, "Fatal: sizeof(unsigned long) too small (%d)\n", s);
    assert(strlen(tmp_buf) + count < err_buf_size - 1);
    strcat(err_buf, tmp_buf);
    count += strlen(tmp_buf);
  }
  if ((s = (int)sizeof(ulonglong)) < 8) {
    (void)sprintf(tmp_buf, "Fatal: sizeof(ulonglong) too small (%d)\n", s);
    assert(strlen(tmp_buf) + count < err_buf_size - 1);
    strcat(err_buf, tmp_buf);
    count += strlen(tmp_buf);
  }
 ...
 */
char *pan_net_start_TX(SOCKET s) {
 ...
 */
char *pan_net_finish_TX(SOCKET s) {
 ...
 */
char *pan_net_get_image_TX(SOCKET s) {
...
unsigned char *pan_net_get_image_RX(SOCKET s, unsigned long *psize) {
  long fsize;
 ...
 */
char *pan_net_get_elevation_TX(SOCKET s) {
...
float pan_net_get_elevation_RX(SOCKET s, char *perr) {
  ...
  if (perr)
    *perr = result.valid ? (char)1 : (char)0;
  return result.value;
}
 ...
 */
char *pan_net_get_elevations_TX(SOCKET s, unsigned long n, float *posv) {
  unsigned long i;
...
void pan_net_get_elevations_RX(SOCKET s, float *resultv, char *errorv) {
 ...
 */
char *pan_net_lookup_point_TX(SOCKET s, float x, float y) {
  /*
...
void pan_net_lookup_point_RX(SOCKET s, float *px, float *py, float *pz,
                             char *perr) {
  char valid;
 ...
 */
char *pan_net_lookup_points_TX(SOCKET s, unsigned long n, float *posv) {
  unsigned long i;
...
void pan_net_lookup_points_RX(SOCKET s, float *resultv, char *errorv) {
  unsigned long i, nelts;
 ...
 */
char *pan_net_get_point_TX(SOCKET s, float dx, float dy, float dz) {
  /*
...
void pan_net_get_point_RX(SOCKET s, float *px, float *py, float *pz,
                          char *perr) {
  char valid;
 ...
 */
char *pan_net_get_points_TX(SOCKET s, unsigned long n, float *posv) {
  unsigned long i;
...
void pan_net_get_points_RX(SOCKET s, float *resultv, char *errorv) {
  unsigned long i, nelts;
 ...
 * IMPLEMENTS Echo (8)
 */
char *pan_net_echo_TX(SOCKET s, void *src, unsigned long n) {
  /*
...
void *pan_net_echo_RX(SOCKET s, unsigned long *psize) {
 ...
 */
char *pan_net_get_range_image_TX(SOCKET s, float offset, float scale) {
  /*
...
unsigned char *pan_net_get_range_image_RX(SOCKET s, unsigned long *psize) {
  long fsize;
  unsigned char *result;
 ...
 */
char *pan_net_get_range_texture_TX(SOCKET s) {
  /* Send empty GetRangeTexture message and want a MSG_IMAGE reply */
  (void)pan_socket_write_ulong(s, MSG_GET_RANGE_TEXTURE);
  return pan_net_want(s, MSG_IMAGE);
}
unsigned char *pan_net_get_range_texture_RX(SOCKET s, unsigned long *psize) {
  long fsize;
  unsigned char *result;
 ...
 */
char *pan_net_get_viewpoint_by_degrees_s_TX(SOCKET s, float x, float y, float z,
                                            float yw, float pi, float rl) {
  /*
...
unsigned char *pan_net_get_viewpoint_by_degrees_s_RX(SOCKET s,
                                                     unsigned long *psize) {
  long fsize;
  unsigned char *result;
 ...
 */
char *pan_net_get_viewpoint_by_quaternion_s_TX(SOCKET s, float x, float y,
                                               float z, float q0, float q1,
                                               float q2, float q3) {
  /*
...
unsigned char *pan_net_get_viewpoint_by_quaternion_s_RX(SOCKET s,
                                                        unsigned long *psize) {
  long fsize;
  unsigned char *result;
 ...
 */
char *pan_net_get_lidar_pulse_result_TX(SOCKET s, float x, float y, float z,
...
void pan_net_get_lidar_pulse_result_RX(SOCKET s, float *pr, float *pa) {
  float v0, v1;
 ...
 */
char *pan_net_get_lidar_measurement_TX(SOCKET s, float px, float py, float pz,
                                       float q0, float q1, float q2, float q3,
                                       float vx, float vy, float vz, float rx,
                                       float ry, float rz, float ax, float ay,
                                       float az, float sx, float sy, float sz,
                                       float jx, float jy, float jz, float tx,
                                       float ty, float tz) {
  /*
...
float *pan_net_get_lidar_measurement_RX(
    SOCKET s, float *pfx, float *pfy, unsigned long *pnx, unsigned long *pny,
    float *ptx, float *pty, unsigned long *pn, unsigned long *pm,
    unsigned long *pt, unsigned long *pfl, float *paz, float *pel, float *pth,
    float *pfaz, float *pfel, float *ptoff, float *ptaz0, float *ptel0) {
  /* Return value */
  float *result, *dptr;
  ...
  (void)pan_socket_read_float(s, &f);
  got++;
  if (pfx)
    *pfx = f;
  (void)pan_socket_read_float(s, &f);
  ...
  if (pfy)
    *pfy = f;
  (void)pan_socket_read_ulong(s, &nx);
  got++;
  if (pnx)
    *pnx = nx;
  (void)pan_socket_read_ulong(s, &ny);
  got++;
  if (pny)
    *pny = ny;
  (void)pan_socket_read_float(s, &f);
  ...
  if (pty)
    *pty = f;
  (void)pan_socket_read_ulong(s, &n);
  got++;
  if (pn)
    *pn = n;
  (void)pan_socket_read_ulong(s, &m);
  got++;
  if (pm)
    *pm = m;
  (void)pan_socket_read_ulong(s, &t);
  got++;
  if (pt)
    *pt = t;
  (void)pan_socket_read_ulong(s, &fl);
  got++;
  if (pfl)
    *pfl = fl;
  (void)pan_socket_read_float(s, &f);
  ...
  for (; got < 32; got++)
    (void)pan_socket_read_ulong(s, &i);
 ...
 */
char *pan_net_get_radar_response_TX(
    SOCKET s, unsigned long flags, unsigned long n, unsigned long nr,
    unsigned long ns, float ox, float oy, float oz, float vx, float vy,
    float vz, float q0, float q1, float q2, float q3, float bwidth, float rmid,
    float smid,          // Ooops: we ought to have swapped these
...
float *pan_net_get_radar_response_RX(
    SOCKET s, unsigned long *pstat, float *pmaxv, float *ptotv, float *poffr,
    float *poffs, float *prbsize, float *psbsize, float *pminr, float *pmaxr,
    float *pmins, float *pmaxs, unsigned long *pnrelts, unsigned long *pnselts,
  ...
  for (i = 0; i < 10; i++)
    (void)pan_socket_read_ulong(s, &junk);
 ...
 */
char *pan_net_get_viewpoint_by_degrees_d_TX(SOCKET s, double x, double y,
                                            double z, double yw, double pi,
                                            double rl) {
  /*
   * Send the GetViewpointByDegrees message with parameters (x, y, z)
...
unsigned char *pan_net_get_viewpoint_by_degrees_d_RX(SOCKET s,
                                                     unsigned long *psize) {
  long fsize;
  unsigned char *result;
 ...
 */
char *pan_net_get_viewpoint_by_quaternion_d_TX(SOCKET s, double x, double y,
                                               double z, double q0, double q1,
                                               double q2, double q3) {
  /*
   * Send the GetViewpointByQuaternion message with parameters (x, y, z)
...
unsigned char *pan_net_get_viewpoint_by_quaternion_d_RX(SOCKET s,
                                                        unsigned long *psize) {
  long fsize;
  unsigned char *result;
 ...
 */
char *pan_net_get_joints_TX(SOCKET s, unsigned long o) {
  /*
...
joint_data *pan_net_get_joints_RX(SOCKET s, unsigned long *n) {
 ...
 */
char *pan_net_get_joint_config_TX(SOCKET s, unsigned long obj,
                                  unsigned long joint) {
  /*
...
void pan_net_get_joint_config_RX(SOCKET s, double *config) {
  unsigned long n;
  char ignored;
  unsigned long i;
 ...
 */
char *pan_net_get_frames_TX(SOCKET s, unsigned long obj) {
  /*
...
frame_data *pan_net_get_frames_RX(SOCKET s, unsigned long *n) {
 ...
 */
char *pan_net_get_frame_TX(SOCKET s, unsigned long obj, unsigned long id) {
  /*
...
void pan_net_get_frame_RX(SOCKET s, double *data) {
  int i;
  unsigned long n;
  char ignored;
 ...
 */
char *pan_net_get_frame_as_radians_TX(SOCKET s, unsigned long obj,
                                      unsigned long id) {
  /*
...
void pan_net_get_frame_as_radians_RX(SOCKET s, double *data) {
  int i;
  unsigned long n;
  char ignored;
 ...
 */
char *pan_net_get_surface_elevation_TX(SOCKET s, unsigned char boulders,
                                       float x, float y) {
  /*
...
float pan_net_get_surface_elevation_RX(SOCKET s, char *err) {
  struct optional_float result;
  ...
  (void)pan_socket_read_bool(s, &result.valid);
  if (err)
    *err = result.valid ? (char)1 : (char)0;
  return result.value;
}
 ...
 */
char *pan_net_get_surface_elevations_TX(SOCKET s, unsigned char boulders,
                                        unsigned long n, float *posv) {
  unsigned long i;
...
void pan_net_get_surface_elevations_RX(SOCKET s, float *resultv, char *errorv) {
  unsigned long i, nelts;
 ...
 */
char *pan_net_get_surface_patch_TX(SOCKET s, unsigned char boulders, float cx,
                                   float cy, unsigned long nx, unsigned long ny,
                                   float d, float theta) {
  /*
...
void pan_net_get_surface_patch_RX(SOCKET s, float *rv, char *ev) {
  unsigned long i, nelts;
 ...
 */
char *pan_net_get_viewpoint_by_radians_TX(SOCKET s, double x, double y,
                                          double z, double yw, double pi,
                                          double rl) {
  /*
...
unsigned char *pan_net_get_viewpoint_by_radians_RX(SOCKET s,
                                                   unsigned long *psize) {
  long fsize;
  unsigned char *result;
 ...
 */
char *pan_net_get_viewpoint_by_frame_TX(SOCKET s, unsigned long oid,
                                        unsigned long fid) {
  /*
...
unsigned char *pan_net_get_viewpoint_by_frame_RX(SOCKET s,
                                                 unsigned long *psize) {
  long fsize;
  unsigned char *result;
 ...
 *
 * v =
 * pan_net_get_camera_properties_RX(s,&w,&h,&hf,&vf,&px,&py,&pz,&q0,&q1,&q2,&3)
 ...
 */
char *pan_net_get_camera_properties_TX(SOCKET s, unsigned long cid) {
  int nbytes;
...
int pan_net_get_camera_properties_RX(SOCKET s, unsigned long *pwidth,
                                     unsigned long *pheight, double *phfov,
                                     double *pvfov, double *px, double *py,
                                     double *pz, double *pq0, double *pq1,
                                     double *pq2, double *pq3) {
  unsigned long n;
  unsigned long want;
  unsigned long got;
  char ignored;
  ...
  for (; got < n; ++got)
    (void)pan_socket_read_char(s, &ignored);
  ...
  if (pwidth)
    pan_socket_peek_ulong(ptr, pwidth);
  ptr += 4;
  /**/
  if (pheight)
    pan_socket_peek_ulong(ptr, pheight);
  ptr += 4;
  /**/
  if (phfov)
    pan_socket_peek_double(ptr, phfov);
  ptr += 8;
  /**/
  if (pvfov)
    pan_socket_peek_double(ptr, pvfov);
  ptr += 8;
  /**/
  if (px)
    pan_socket_peek_double(ptr, px);
  ptr += 8;
  /**/
  if (py)
    pan_socket_peek_double(ptr, py);
  ptr += 8;
  /**/
  if (pz)
    pan_socket_peek_double(ptr, pz);
  ptr += 8;
  /**/
  if (pq0)
    pan_socket_peek_double(ptr, pq0);
  ptr += 8;
  /**/
  if (pq1)
    pan_socket_peek_double(ptr, pq1);
  ptr += 8;
  /**/
  if (pq2)
    pan_socket_peek_double(ptr, pq2);
  ptr += 8;
  /**/
  if (pq3)
    pan_socket_peek_double(ptr, pq3);
  ptr += 8;
 ...
 */
char *pan_net_get_viewpoint_by_camera_TX(SOCKET s, unsigned long cid) {
  /*
...
unsigned char *pan_net_get_viewpoint_by_camera_RX(SOCKET s,
                                                  unsigned long *psize) {
  long fsize;
  unsigned char *result;
 ...
 */
char *pan_net_get_view_as_dem_TX(SOCKET s, unsigned long cid,
                                 unsigned char boulders, unsigned long nx,
                                 unsigned long ny, float dx, float dy,
                                 float rd) {
  /*
...
void pan_net_get_view_as_dem_RX(SOCKET s, float *rv, char *ev) {
  unsigned long i, nelts;
 ...
 */
char *pan_net_get_lidar_measurement_d_TX(
    SOCKET s, double px, double py, double pz, double q0, double q1, double q2,
    double q3, double vx, double vy, double vz, double rx, double ry, double rz,
    double ax, double ay, double az, double sx, double sy, double sz, double jx,
    double jy, double jz, double tx, double ty, double tz) {
  /*
   * Send the GetLidarMeasurement message.
...
float *pan_net_get_lidar_measurement_d_RX(
    SOCKET s, float *pfx, float *pfy, unsigned long *pnx, unsigned long *pny,
    float *ptx, float *pty, unsigned long *pn, unsigned long *pm,
    unsigned long *pt, unsigned long *pfl, float *paz, float *pel, float *pth,
    float *pfaz, float *pfel, float *ptoff, float *ptaz0, float *ptel0) {
 ...
 */
char *pan_net_get_time_tag_TX(SOCKET s) {
...
double pan_net_get_time_tag_RX(SOCKET s, char *perr) {
 ...
 */
char *pan_net_get_lidar_measurement_s_TX(SOCKET s, float px, float py, float pz,
                                         float q0, float q1, float q2, float q3,
                                         float vx, float vy, float vz, float rx,
                                         float ry, float rz, float ax, float ay,
                                         float az, float sx, float sy, float sz,
...
float *pan_net_get_lidar_measurement_s_RX(
    SOCKET s, float *pfx, float *pfy, unsigned long *pnx, unsigned long *pny,
    float *ptx, float *pty, unsigned long *pn, unsigned long *pm,
    unsigned long *pt, unsigned long *pfl, float *paz, float *pel, float *pth,
    float *pfaz, float *pfel, float *ptoff, float *ptaz0, float *ptel0) {
 ...
 */
char *pan_net_get_lidar_snapshot_TX(SOCKET s, unsigned long cid, double px,
                                    double py, double pz, double q0, double q1,
                                    double q2, double q3) {
  /*
...
float *pan_net_get_lidar_snapshot_RX(SOCKET s, unsigned long *width,
  ...
  (void)pan_socket_read_ulong(s, &w);
  if (width)
    *width = w;
  (void)pan_socket_read_ulong(s, &h);
  if (height)
    *height = h;
 ...
 */
char *pan_net_set_viewpoint_by_degrees_s_TX(SOCKET s, float x, float y, float z,
                                            float yw, float pi, float rl) {
  /*
 ...
 */
char *pan_net_set_viewpoint_by_quaternion_s_TX(SOCKET s, float x, float y,
                                               float z, float q0, float q1,
                                               float q2, float q3) {
  /*
 ...
 */
char *pan_net_set_ambient_light_TX(SOCKET s, float r, float g, float b) {
  /*
 ...
 */
char *pan_net_set_sun_colour_TX(SOCKET s, float r, float g, float b) {
  /*
 ...
 */
char *pan_net_set_sky_type_TX(SOCKET s, unsigned long t) {
  /*
 ...
 */
char *pan_net_set_field_of_view_by_degrees_TX(SOCKET s, float f) {
  /*
 ...
 */
char *pan_net_set_aspect_ratio_TX(SOCKET s, float r) {
  /*
 ...
 */
char *pan_net_set_boulder_view_TX(SOCKET s, unsigned long type, int texture) {
  /*
 ...
 */
char *pan_net_set_surface_view_TX(SOCKET s, unsigned long type, int tex,
                                  int det) {
  /*
 ...
 */
char *pan_net_set_lidar_parameters_TX(
    SOCKET s, float fx, float fy, unsigned long nx, unsigned long ny, float tx,
    float ty, unsigned long n, unsigned long m, unsigned long t,
    unsigned long fl, float az, float el, float th, float wx, float wy,
    float faz, float fel, float toff, float taz0, float tel0) {
  int wrote = 0;
  ...
  p = pan_socket_poke_float(p, fx);
  wrote++;
  p = pan_socket_poke_float(p, fy);
  wrote++;
  p = pan_socket_poke_ulong(p, nx);
  wrote++;
  p = pan_socket_poke_ulong(p, ny);
  ...
  p = pan_socket_poke_ulong(p, n);
  wrote++;
  p = pan_socket_poke_ulong(p, m);
  wrote++;
  p = pan_socket_poke_ulong(p, t);
  wrote++;
  p = pan_socket_poke_ulong(p, fl);
  wrote++;
  p = pan_socket_poke_float(p, az);
  wrote++;
  p = pan_socket_poke_float(p, el);
  wrote++;
  p = pan_socket_poke_float(p, th);
  wrote++;
  p = pan_socket_poke_float(p, wx);
  wrote++;
  p = pan_socket_poke_float(p, wy);
  wrote++;
  p = pan_socket_poke_float(p, faz);
  wrote++;
  p = pan_socket_poke_float(p, fel);
  wrote++;
  p = pan_socket_poke_float(p, toff);
  wrote++;
  p = pan_socket_poke_float(p, taz0);
  wrote++;
  p = pan_socket_poke_float(p, tel0);
  wrote++;

  /* Padding words */
  for (i = wrote; i < 32; i++)
    p = pan_socket_poke_ulong(p, zero);
  nbytes = p - buf;
 ...
 */
char *pan_net_set_corner_cubes_s_TX(SOCKET s, unsigned long n,
                                    unsigned long fmt, float *pcc) {
  unsigned long i, siz;
 ...
 */
char *pan_net_set_corner_cube_attitude_TX(SOCKET s, float q0, float q1,
                                          float q2, float q3, float rx,
                                          float ry, float rz, float ax,
                                          float ay, float az, float jx,
                                          float jy, float jz) {
  /*
 ...
 */
char *pan_net_set_viewpoint_by_degrees_d_TX(SOCKET s, double x, double y,
                                            double z, double yw, double pi,
                                            double rl) {
  /*
   * Send the SetViewpointByDegrees message with parameters (x, y, z)
 ...
 */
char *pan_net_set_viewpoint_by_quaternion_d_TX(SOCKET s, double x, double y,
                                               double z, double q0, double q1,
                                               double q2, double q3) {
  /*
   * Send the SetViewpointByQuaternion message with parameters (x, y, z)
 ...
 */
char *pan_net_set_object_position_attitude_TX(SOCKET s, unsigned long id,
 ...
 */
char *pan_net_set_sun_by_degrees_TX(SOCKET s, double r, double a, double e) {
  /*
 ...
 */
char *pan_net_set_joint_config_TX(SOCKET s, unsigned long obj,
                                  unsigned long joint, double config[9]) {
  int i;
 ...
 */
char *pan_net_set_star_quaternion_TX(SOCKET s, double q0, double q1, double q2,
                                     double q3) {
  /*
 ...
 */
char *pan_net_set_star_magnitudes_TX(SOCKET s, double m) {
  /*
 ...
 */
char *pan_net_set_secondary_by_degrees_TX(SOCKET s, double r, double a,
                                          double e) {
  /*
 ...
 */
char *pan_net_set_global_time_TX(SOCKET s, double t) {
  /*
   * Send the SetStarMagnitudes message.
   * We're sending one ulong and one double.
   */
 ...
 */
char *pan_net_set_object_view_TX(SOCKET s, unsigned long id,
                                 unsigned long type) {
  /*
 ...
 */
char *pan_net_set_viewpoint_by_radians_TX(SOCKET s, double x, double y,
                                          double z, double yw, double pi,
                                          double rl) {
  /*
 ...
 */
char *pan_net_set_field_of_view_by_radians_TX(SOCKET s, float f) {
  /*
 ...
 */
char *pan_net_set_sun_by_radians_TX(SOCKET s, double r, double a, double e) {
  /*
 ...
 */
char *pan_net_set_secondary_by_radians_TX(SOCKET s, double r, double a,
                                          double e) {
  /*
 ...
 */
char *pan_net_set_sky_rgb_TX(SOCKET s, float r, float g, float b) {
  /*
 ...
 */
char *pan_net_set_sky_cie_TX(SOCKET s, float x, float y, float Y) {
  /*
 ...
 */
char *pan_net_set_atmosphere_tau_TX(SOCKET s, float mr, float mg, float mb,
 ...
 */
char *pan_net_set_global_fog_mode_TX(SOCKET s, unsigned long mode) {
  /*
 ...
 */
char *pan_net_set_global_fog_properties_TX(SOCKET s, double radius,
                                           double density, double lin0,
                                           double lin1) {
  /*
 ...
 */
char *pan_net_set_atmosphere_mode_TX(SOCKET s, unsigned long smode,
                                     unsigned long gmode, unsigned long amode) {
  /*
 ...
 */
char *pan_net_select_camera_TX(SOCKET s, unsigned long cid) {
  int nbytes;
 ...
 */
char *pan_net_bind_light_to_camera_TX(SOCKET s, unsigned long lid,
                                      unsigned long cid, unsigned char en) {
  int nbytes;
 ...
 */
char *pan_net_configure_light_by_degrees_TX(SOCKET s, unsigned long lid,
  ...
  /* Padding */
  for (int i = 0; i < 10; ++i)
    p = pan_socket_poke_float(p, 0.0f);
  nbytes = p - buf;
 ...
 */
char *pan_net_configure_light_by_radians_TX(SOCKET s, unsigned long lid,
                                            double r, double g, double b,
                                            double h, double e) {
  int nbytes;
 ...
 */
char *pan_net_set_light_position_direction_TX(SOCKET s, unsigned long lid,
 ...
 */
char *pan_net_render_to_hold_buffer_TX(SOCKET s, unsigned long cid,
                                       unsigned long bid) {
  int nbytes;
 ...
 */
char *pan_net_display_hold_buffer_TX(SOCKET s, unsigned long bid) {
  int nbytes;
 ...
 */
char *pan_net_set_corner_cubes_d_TX(SOCKET s, unsigned long n,
                                    unsigned long fmt, double *pcc) {
  unsigned long i, siz;
  ...
  for (i = 0; i < 7 * n; i++)
    p = pan_socket_poke_double(p, *pcc++);
  nbytes = p - buf;
 ...
 */
char *pan_net_set_projection_mode_TX(SOCKET s, unsigned long cid,
                                     unsigned long mode) {
  int nbytes;
 ...
 */
char *pan_net_set_ortho_field_of_view_TX(SOCKET s, unsigned long cid,
 ...
 */
char *pan_net_set_lidar_scan_TX(SOCKET s, unsigned long n, unsigned long fmt,
  ...
  for (i = 0; i < 8 * n; i++)
    p = pan_socket_poke_double(p, *pls++);
  nbytes = p - buf;
 ...
 */
char *pan_net_set_camera_motion_TX(
    SOCKET s, unsigned long cid, double vx, double vy,
    double vz,                       /* linear velocity */
 ...
 */
void pan_protocol_expect(SOCKET s, unsigned long want) {
 ...
 */
PanguResponse pan_protocol_start(SOCKET s) {
  char *error = pan_net_start_TX(s);
  if (!error)
    return PanguResponse::MSG_OKAY;
  (void)fprintf(stderr, "%s", error);
  return PanguResponse::MSG_ERROR;
}
 ...
 * IMPLEMENTS Goodbye (0)
 */
void pan_protocol_finish(SOCKET s) { (void)pan_net_finish_TX(s); }

/*
 * ptr = pan_protocol_get_image(s, &size, &img) requests an image from the
 * remote server using the current camera settings. Returns PanguResponse. The
 * memory returned by the call is allocated by malloc() and may be released by
 * free(). The size field will be updated with the number of bytes in the result
 * array.
 *
 * IMPLEMENTS GetImage (1)
 */
PanguResponse pan_protocol_get_image(SOCKET s, unsigned long *psize,
                                     unsigned char *&img) {
  char *error = pan_net_get_image_TX(s);
  if (!error) {
    img = pan_net_get_image_RX(s, psize);
    return PanguResponse::MSG_OKAY;
  }
  (void)fprintf(stderr, "%s", error);
  return PanguResponse::MSG_ERROR;
 ...
 * IMPLEMENTS GetElevation (2)
 */
float pan_protocol_get_elevation(SOCKET s, char *perr) {
  char *error = pan_net_get_elevation_TX(s);
  if (!error)
    return pan_net_get_elevation_RX(s, perr);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS GetElevations (3)
 */
void pan_protocol_get_elevations(SOCKET s, unsigned long n, float *posv,
                                 float *resultv, char *errorv) {
  char *error = pan_net_get_elevations_TX(s, n, posv);
  if (!error)
    return pan_net_get_elevations_RX(s, resultv, errorv);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS LookupPoint (4)
 */
void pan_protocol_lookup_point(SOCKET s, float x, float y, float *px, float *py,
                               float *pz, char *perr) {
  char *error = pan_net_lookup_point_TX(s, x, y);
  if (!error)
    return pan_net_lookup_point_RX(s, px, py, pz, perr);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS LookupPoints (5)
 */
void pan_protocol_lookup_points(SOCKET s, unsigned long n, float *posv,
                                float *resultv, char *errorv) {
  char *error = pan_net_lookup_points_TX(s, n, posv);
  if (!error)
    return pan_net_lookup_points_RX(s, resultv, errorv);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS GetPoint (6)
 */
void pan_protocol_get_point(SOCKET s, float dx, float dy, float dz, float *px,
                            float *py, float *pz, char *perr) {
  char *error = pan_net_get_point_TX(s, dx, dy, dz);
  if (!error)
    return pan_net_get_point_RX(s, px, py, pz, perr);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS GetPoints (7)
 */
void pan_protocol_get_points(SOCKET s, unsigned long n, float *posv,
                             float *resultv, char *errorv) {
  char *error = pan_net_get_points_TX(s, n, posv);
  if (!error)
    return pan_net_get_points_RX(s, resultv, errorv);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS Echo (8)
 */
void *pan_protocol_echo(SOCKET s, void *src, unsigned long n,
                        unsigned long *psize) {
  char *error = pan_net_echo_TX(s, src, n);
  if (!error)
    return pan_net_echo_RX(s, psize);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS GetRangeImage (9)
 */
unsigned char *pan_protocol_get_range_image(SOCKET s, unsigned long *psize,
                                            float offset, float scale) {
  char *error = pan_net_get_range_image_TX(s, offset, scale);
  if (!error)
    return pan_net_get_range_image_RX(s, psize);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS GetRangeTexture (10)
 */
unsigned char *pan_protocol_get_range_texture(SOCKET s, unsigned long *psize) {
  char *error = pan_net_get_range_texture_TX(s);
  if (!error)
    return pan_net_get_range_texture_RX(s, psize);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS GetViewpointByDegreesS (11)
 */
unsigned char *pan_protocol_get_viewpoint_by_degrees_s(SOCKET s, float x,
                                                       float y, float z,
                                                       float yw, float pi,
                                                       float rl,
                                                       unsigned long *psize) {
  ...
  if (!error)
    return pan_net_get_viewpoint_by_degrees_s_RX(s, psize);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 */
unsigned char *pan_protocol_get_viewpoint_by_angle(SOCKET s, float x, float y,
                                                   float z, float yw, float pi,
                                                   float rl,
                                                   unsigned long *psize) {
 ...
 */
unsigned char *pan_protocol_get_viewpoint_by_angle_s(SOCKET s, float x, float y,
                                                     float z, float yw,
                                                     float pi, float rl,
                                                     unsigned long *sz) {
 ...
 */
unsigned char *
pan_protocol_get_viewpoint_by_quaternion_s(SOCKET s, float x, float y, float z,
                                           float q0, float q1, float q2,
                                           float q3, unsigned long *psize) {
  char *error =
      pan_net_get_viewpoint_by_quaternion_s_TX(s, x, y, z, q0, q1, q2, q3);
  if (!error)
    return pan_net_get_viewpoint_by_quaternion_s_RX(s, psize);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 */
unsigned char *pan_protocol_get_viewpoint_by_quaternion(SOCKET s, float x,
                                                        float y, float z,
                                                        float q0, float q1,
                                                        float q2, float q3,
                                                        unsigned long *psize) {
  return pan_protocol_get_viewpoint_by_quaternion_s(s, x, y, z, q0, q1, q2, q3,
                                                    psize);
}
 ...
 * IMPLEMENTS GetLidarPulseResult (13)
 */
void pan_protocol_get_lidar_pulse_result(SOCKET s, float x, float y, float z,
                                         float dx, float dy, float dz,
                                         float *pr, float *pa) {
  char *error = pan_net_get_lidar_pulse_result_TX(s, x, y, z, dx, dy, dz);
  if (!error)
    return pan_net_get_lidar_pulse_result_RX(s, pr, pa);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS GetLidarMeasurement (14)
 */
float *pan_protocol_get_lidar_measurement(
    SOCKET s, float px, float py, float pz, float q0, float q1, float q2,
    float q3, float vx, float vy, float vz, float rx, float ry, float rz,
    float ax, float ay, float az, float sx, float sy, float sz, float jx,
    float jy, float jz, float tx, float ty, float tz, float *pfx, float *pfy,
    unsigned long *pnx, unsigned long *pny, float *ptx, float *pty,
    unsigned long *pn, unsigned long *pm, unsigned long *pt, unsigned long *pfl,
    float *paz, float *pel, float *pth, float *pfaz, float *pfel, float *ptoff,
    float *ptaz0, float *ptel0) {
  char *error = pan_net_get_lidar_measurement_TX(
      s, px, py, pz, q0, q1, q2, q3, vx, vy, vz, rx, ry, rz, ax, ay, az, sx, sy,
      sz, jx, jy, jz, tx, ty, tz);
  if (!error)
    return pan_net_get_lidar_measurement_RX(s, pfx, pfy, pnx, pny, ptx, pty, pn,
                                            pm, pt, pfl, paz, pel, pth, pfaz,
                                            pfel, ptoff, ptaz0, ptel0);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
...
float *pan_protocol_get_radar_response(
    ...
    float smid,           // Ooops: we ought to have swapped these
    float rbs, float sbs, // two lines to match PROTOCOL.txt
    unsigned long *pstat, float *pmaxv, float *ptotv, float *poffr,
    float *poffs, float *prbsize, float *psbsize, float *pminr, float *pmaxr,
    float *pmins, float *pmaxs, unsigned long *pnrelts, unsigned long *pnselts,
    unsigned long *pnused) {
  char *error = pan_net_get_radar_response_TX(s, flags, n, nr, ns, ox, oy, oz,
                                              vx, vy, vz, q0, q1, q2, q3,
                                              bwidth, rmid, smid, rbs, sbs);
  if (!error)
    return pan_net_get_radar_response_RX(s, pstat, pmaxv, ptotv, poffr, poffs,
                                         prbsize, psbsize, pminr, pmaxr, pmins,
                                         pmaxs, pnrelts, pnselts, pnused);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS GetViewpointByDegreesD (16)
 */
unsigned char *pan_protocol_get_viewpoint_by_degrees_d(SOCKET s, double x,
                                                       double y, double z,
                                                       double yw, double pi,
                                                       double rl,
                                                       unsigned long *psize) {
  ...
  if (!error)
    return pan_net_get_viewpoint_by_degrees_d_RX(s, psize);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 */
unsigned char *pan_protocol_get_viewpoint_by_angle_d(SOCKET s, double x,
                                                     double y, double z,
                                                     double yw, double pi,
                                                     double rl,
                                                     unsigned long *sz) {
 ...
 * IMPLEMENTS GetViewpointByQuaternionD (17)
 */
unsigned char *pan_protocol_get_viewpoint_by_quaternion_d(
    SOCKET s, double x, double y, double z, double q0, double q1, double q2,
    double q3, unsigned long *psize) {
  char *error =
      pan_net_get_viewpoint_by_quaternion_d_TX(s, x, y, z, q0, q1, q2, q3);
  if (!error)
    return pan_net_get_viewpoint_by_quaternion_d_RX(s, psize);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS GetJoints (18)
 */
joint_data *pan_protocol_get_joints(SOCKET s, unsigned long o,
                                    unsigned long *n) {
  char *error = pan_net_get_joints_TX(s, o);
  if (!error)
    return pan_net_get_joints_RX(s, n);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS GetJointConfig (19)
 */
void pan_protocol_get_joint_config(SOCKET s, unsigned long obj,
                                   unsigned long joint, double *config) {
  char *error = pan_net_get_joint_config_TX(s, obj, joint);
  if (!error)
    return pan_net_get_joint_config_RX(s, config);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS GetFrames (20)
 */
frame_data *pan_protocol_get_frames(SOCKET s, unsigned long obj,
                                    unsigned long *n) {
  char *error = pan_net_get_frames_TX(s, obj);
  if (!error)
    return pan_net_get_frames_RX(s, n);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS GetFrame (21)
 */
void pan_protocol_get_frame(SOCKET s, unsigned long obj, unsigned long id,
                            double *data) {
  char *error = pan_net_get_frame_TX(s, obj, id);
  if (!error)
    return pan_net_get_frame_RX(s, data);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS GetFrameAsRadians (22)
 */
void pan_protocol_get_frame_as_radians(SOCKET s, unsigned long obj,
                                       unsigned long id, double *data) {
  char *error = pan_net_get_frame_as_radians_TX(s, obj, id);
  if (!error)
    return pan_net_get_frame_as_radians_RX(s, data);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 */
void pan_protocol_get_frame_viewpoint_by_angle(SOCKET s, unsigned long o,
                                               unsigned long i, double *v) {
 ...
 * IMPLEMENTS GetSurfaceElevation (23)
 */
float pan_protocol_get_surface_elevation(SOCKET s, unsigned char boulders,
                                         float x, float y, char *err) {
  char *error = pan_net_get_surface_elevation_TX(s, boulders, x, y);
  if (!error)
    return pan_net_get_surface_elevation_RX(s, err);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS GetSurfaceElevations (24)
 */
void pan_protocol_get_surface_elevations(SOCKET s, unsigned char boulders,
                                         unsigned long n, float *posv,
                                         float *resultv, char *errorv) {
  char *error = pan_net_get_surface_elevations_TX(s, boulders, n, posv);
  if (!error)
    return pan_net_get_surface_elevations_RX(s, resultv, errorv);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS GetSurfacePatch (25)
 */
void pan_protocol_get_surface_patch(SOCKET s, unsigned char boulders, float cx,
                                    float cy, unsigned long nx,
                                    unsigned long ny, float d, float theta,
                                    float *rv, char *ev) {
  char *error =
      pan_net_get_surface_patch_TX(s, boulders, cx, cy, nx, ny, d, theta);
  if (!error)
    return pan_net_get_surface_patch_RX(s, rv, ev);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS GetViewpointByRadians (26)
 */
unsigned char *pan_protocol_get_viewpoint_by_radians(SOCKET s, double x,
                                                     double y, double z,
                                                     double yw, double pi,
                                                     double rl,
                                                     unsigned long *psize) {
  ...
  if (!error)
    return pan_net_get_viewpoint_by_radians_RX(s, psize);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS Quit (27)
 */
void pan_protocol_quit(SOCKET s) {
 ...
 * IMPLEMENTS GetViewpointByFrame (28)
 */
unsigned char *pan_protocol_get_viewpoint_by_frame(SOCKET s, unsigned long oid,
                                                   unsigned long fid,
                                                   unsigned long *psize) {
  char *error = pan_net_get_viewpoint_by_frame_TX(s, oid, fid);
  if (!error)
    return pan_net_get_viewpoint_by_frame_RX(s, psize);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
...
/*
 * v =
 * pan_protocol_get_camera_properties(s,c,&w,&h,&hf,&vf,&px,&py,&pz,&q0,&q1,&q2,&3)
 ...
 * IMPLEMENTS GetCameraProperties (29)
 */
int pan_protocol_get_camera_properties(SOCKET s, unsigned long cid,
  ...
  if (!error)
    return pan_net_get_camera_properties_RX(s, pwidth, pheight, phfov, pvfov,
                                            px, py, pz, pq0, pq1, pq2, pq3);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS GetViewpointByCamera (30)
 */
unsigned char *pan_protocol_get_viewpoint_by_camera(SOCKET s, unsigned long cid,
                                                    unsigned long *psize) {
  char *error = pan_net_get_viewpoint_by_camera_TX(s, cid);
  if (!error)
    return pan_net_get_viewpoint_by_camera_RX(s, psize);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS GetViewAsDEM (31)
 */
void pan_protocol_get_view_as_dem(SOCKET s, unsigned long cid,
                                  unsigned char boulders, unsigned long nx,
                                  unsigned long ny, float dx, float dy,
                                  float rd, float *rv, char *ev) {
  char *error =
      pan_net_get_view_as_dem_TX(s, cid, boulders, nx, ny, dx, dy, rd);
  if (!error)
    return pan_net_get_view_as_dem_RX(s, rv, ev);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS GetLidarMeasurementD (32)
 */
float *pan_protocol_get_lidar_measurement_d(
    SOCKET s, double px, double py, double pz, double q0, double q1, double q2,
    double q3, double vx, double vy, double vz, double rx, double ry, double rz,
    double ax, double ay, double az, double sx, double sy, double sz, double jx,
    double jy, double jz, double tx, double ty, double tz, float *pfx,
    float *pfy, unsigned long *pnx, unsigned long *pny, float *ptx, float *pty,
    unsigned long *pn, unsigned long *pm, unsigned long *pt, unsigned long *pfl,
    float *paz, float *pel, float *pth, float *pfaz, float *pfel, float *ptoff,
    float *ptaz0, float *ptel0) {
  char *error = pan_net_get_lidar_measurement_d_TX(
      s, px, py, pz, q0, q1, q2, q3, vx, vy, vz, rx, ry, rz, ax, ay, az, sx, sy,
      sz, jx, jy, jz, tx, ty, tz);
  if (!error)
    return pan_net_get_lidar_measurement_d_RX(s, pfx, pfy, pnx, pny, ptx, pty,
                                              pn, pm, pt, pfl, paz, pel, pth,
                                              pfaz, pfel, ptoff, ptaz0, ptel0);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS GetTimeTag (33)
 */
double pan_protocol_get_time_tag(SOCKET s, char *perr) {
  char *error = pan_net_get_time_tag_TX(s);
  if (!error)
    return pan_net_get_time_tag_RX(s, perr);
 ...
 * IMPLEMENTS GetLidarMeasurementS (34)
 */
float *pan_protocol_get_lidar_measurement_s(
    SOCKET s, float px, float py, float pz, float q0, float q1, float q2,
    float q3, float vx, float vy, float vz, float rx, float ry, float rz,
    float ax, float ay, float az, float sx, float sy, float sz, float jx,
    float jy, float jz, float tx, float ty, float tz, float *pfx, float *pfy,
    ...
    float *paz, float *pel, float *pth, float *pfaz, float *pfel, float *ptoff,
    float *ptaz0, float *ptel0) {
  char *error = pan_net_get_lidar_measurement_s_TX(
      s, px, py, pz, q0, q1, q2, q3, vx, vy, vz, rx, ry, rz, ax, ay, az, sx, sy,
      sz, jx, jy, jz, tx, ty, tz);
  if (!error)
    return pan_net_get_lidar_measurement_s_RX(s, pfx, pfy, pnx, pny, ptx, pty,
                                              pn, pm, pt, pfl, paz, pel, pth,
                                              pfaz, pfel, ptoff, ptaz0, ptel0);
 ...
 * IMPLEMENTS GetLidarSnapshot (35)
 */
float *pan_protocol_get_lidar_snapshot(SOCKET s, unsigned long cid, double px,
                                       double py, double pz, double q0,
                                       double q1, double q2, double q3,
                                       unsigned long *width,
                                       unsigned long *height) {
  char *error =
      pan_net_get_lidar_snapshot_TX(s, cid, px, py, pz, q0, q1, q2, q3);
  if (!error)
    return pan_net_get_lidar_snapshot_RX(s, width, height);
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
}
 ...
 * IMPLEMENTS SetViewpointByDegreesS (256)
 */
PanguResponse pan_protocol_set_viewpoint_by_degrees_s(SOCKET s, float x,
                                                      float y, float z,
                                                      float yw, float pi,
                                                      float rl) {
 ...
 */
PanguResponse pan_protocol_set_viewpoint_by_angle(SOCKET s, float x, float y,
                                                  float z, float yw, float pi,
                                                  float rl) {
  return pan_protocol_set_viewpoint_by_degrees_s(s, x, y, z, yw, pi, rl);
}
 ...
 */
PanguResponse pan_protocol_set_viewpoint_by_angle_s(SOCKET s, float x, float y,
                                                    float z, float yw, float pi,
                                                    float rl) {
  return pan_protocol_set_viewpoint_by_degrees_s(s, x, y, z, yw, pi, rl);
}
 ...
 * IMPLEMENTS SetViewpointByQuaternionS (257)
 */
PanguResponse pan_protocol_set_viewpoint_by_quaternion_s(SOCKET s, float x,
                                                         float y, float z,
                                                         float q0, float q1,
                                                         float q2, float q3) {
  char *error =
      pan_net_set_viewpoint_by_quaternion_s_TX(s, x, y, z, q0, q1, q2, q3);
  if (!error)
    return PanguResponse::MSG_OKAY;
  (void)fprintf(stderr, "%s", error);
  return PanguResponse::MSG_ERROR;
 ...
 */
PanguResponse pan_protocol_set_viewpoint_by_quaternion(SOCKET s, float x,
                                                       float y, float z,
                                                       float q0, float q1,
                                                       float q2, float q3) {
  return pan_protocol_set_viewpoint_by_quaternion_s(s, x, y, z, q0, q1, q2, q3);
}
 ...
 * IMPLEMENTS SetAmbientLight (258)
 */
void pan_protocol_set_ambient_light(SOCKET s, float r, float g, float b) {
 ...
 * IMPLEMENTS SetSunColour (259)
 */
void pan_protocol_set_sun_colour(SOCKET s, float r, float g, float b) {
 ...
 * IMPLEMENTS SetSkyType (260)
 */
void pan_protocol_set_sky_type(SOCKET s, unsigned long t) {
 ...
 * IMPLEMENTS SetFieldOfViewByDegrees (261)
 */
void pan_protocol_set_field_of_view_by_degrees(SOCKET s, float f) {
 ...
 * IMPLEMENTS SetAspectRatio (262)
 */
void pan_protocol_set_aspect_ratio(SOCKET s, float r) {
 ...
 * IMPLEMENTS SetBoulderView (263)
 */
void pan_protocol_set_boulder_view(SOCKET s, unsigned long type, int texture) {
 ...
 * IMPLEMENTS SetSurfaceView (264)
 */
void pan_protocol_set_surface_view(SOCKET s, unsigned long type, int tex,
                                   int det) {
 ...
 * IMPLEMENTS SetLidarParameters (265)
 */
void pan_protocol_set_lidar_parameters(
    SOCKET s, float fx, float fy, unsigned long nx, unsigned long ny, float tx,
    float ty, unsigned long n, unsigned long m, unsigned long t,
    unsigned long fl, float az, float el, float th, float wx, float wy,
    float faz, float fel, float toff, float taz0, float tel0) {
  char *error = pan_net_set_lidar_parameters_TX(s, fx, fy, nx, ny, tx, ty, n, m,
                                                t, fl, az, el, th, wx, wy, faz,
                                                fel, toff, taz0, tel0);
  if (!error)
    return;
  (void)fprintf(stderr, "%s", error);
 ...
 * IMPLEMENTS SetCornerCubesS (266)
 */
void pan_protocol_set_corner_cubes_s(SOCKET s, unsigned long n,
                                     unsigned long fmt, float *pcc) {
 ...
 * IMPLEMENTS SetCornerCubesS (266)
 */
void pan_protocol_set_corner_cubes(SOCKET s, unsigned long n, unsigned long fmt,
                                   float *pcc) {
 ...
 * IMPLEMENTS SetCornerCubeAttitude (267)
 */
void pan_protocol_set_corner_cube_attitude(SOCKET s, float q0, float q1,
                                           float q2, float q3, float rx,
                                           float ry, float rz, float ax,
                                           float ay, float az, float jx,
                                           float jy, float jz) {
  char *error = pan_net_set_corner_cube_attitude_TX(s, q0, q1, q2, q3, rx, ry,
                                                    rz, ax, ay, az, jx, jy, jz);
  if (!error)
    return;
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
 ...
 * IMPLEMENTS SetViewpointByDegreesD (268)
 */
PanguResponse pan_protocol_set_viewpoint_by_degrees_d(SOCKET s, double x,
                                                      double y, double z,
                                                      double yw, double pi,
                                                      double rl) {
 ...
 */
PanguResponse pan_protocol_set_viewpoint_by_angle_d(SOCKET s, double x,
                                                    double y, double z,
                                                    double yw, double pi,
                                                    double rl) {
  return pan_protocol_set_viewpoint_by_degrees_d(s, x, y, z, yw, pi, rl);
}
 ...
 * IMPLEMENTS SetViewpointByQuaternionD (269)
 */
PanguResponse pan_protocol_set_viewpoint_by_quaternion_d(SOCKET s, double x,
                                                         double y, double z,
                                                         double q0, double q1,
                                                         double q2, double q3) {
  char *error =
      pan_net_set_viewpoint_by_quaternion_d_TX(s, x, y, z, q0, q1, q2, q3);
  if (!error)
    return PanguResponse::MSG_OKAY;
  (void)fprintf(stderr, "%s", error);
  return PanguResponse::MSG_ERROR;
 ...
 * IMPLEMENTS SetObjectPositionAttitude (270)
 */
void pan_protocol_set_object_position_attitude(SOCKET s, unsigned long id,
                                               double x, double y, double z,
                                               double q0, double q1, double q2,
                                               double q3) {
  char *error =
      pan_net_set_object_position_attitude_TX(s, id, x, y, z, q0, q1, q2, q3);
  if (!error)
    return;
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
 ...
 * IMPLEMENTS SetObjectPositionAttitude (270)
 */
void pan_protocol_set_object_position(SOCKET s, unsigned long id, double x,
                                      double y, double z, double q0, double q1,
                                      double q2, double q3) {
 ...
 * IMPLEMENTS SetSunByDegrees (271)
 */
void pan_protocol_set_sun_by_degrees(SOCKET s, double r, double a, double e) {
 ...
 */
void pan_protocol_set_sun_position(SOCKET s, double r, double a, double e) {
 ...
 * IMPLEMENTS SetJointConfig (272)
 */
void pan_protocol_set_joint_config(SOCKET s, unsigned long obj,
                                   unsigned long joint, double config[9]) {
 ...
 * IMPLEMENTS SetStarQuaternion (273)
 */
void pan_protocol_set_star_quaternion(SOCKET s, double q0, double q1, double q2,
                                      double q3) {
 ...
 * IMPLEMENTS SetStarMagnitudes (274)
 */
void pan_protocol_set_star_magnitudes(SOCKET s, double m) {
 ...
 * IMPLEMENTS SetSecondaryByDegrees (275)
 */
void pan_protocol_set_secondary_by_degrees(SOCKET s, double r, double a,
                                           double e) {
 ...
 * IMPLEMENTS SetGlobalTime (276)
 */
void pan_protocol_set_global_time(SOCKET s, double t) {
 ...
 * IMPLEMENTS SetObjectView (277)
 */
void pan_protocol_set_object_view(SOCKET s, unsigned long id,
                                  unsigned long type) {
 ...
 * IMPLEMENTS SetViewpointByRadians (278)
 */
PanguResponse pan_protocol_set_viewpoint_by_radians(SOCKET s, double x,
                                                    double y, double z,
                                                    double yw, double pi,
                                                    double rl) {
 ...
 * IMPLEMENTS SetFieldOfViewByRadians (279)
 */
void pan_protocol_set_field_of_view_by_radians(SOCKET s, float f) {
 ...
 * IMPLEMENTS SetSunByRadians (280)
 */
void pan_protocol_set_sun_by_radians(SOCKET s, double r, double a, double e) {
 ...
 * IMPLEMENTS SetSecondaryByRadians (281)
 */
void pan_protocol_set_secondary_by_radians(SOCKET s, double r, double a,
                                           double e) {
 ...
 * IMPLEMENTS SetSkyRGB (282)
 */
void pan_protocol_set_sky_rgb(SOCKET s, float r, float g, float b) {
 ...
 * IMPLEMENTS SetSkyCIE (283)
 */
void pan_protocol_set_sky_cie(SOCKET s, float x, float y, float Y) {
 ...
 * IMPLEMENTS SetAtmosphereTau (284)
 */
void pan_protocol_set_atmosphere_tau(SOCKET s, float mr, float mg, float mb,
                                     float rr, float rg, float rb) {
 ...
 * IMPLEMENTS SetGlobalFogMode (285)
 */
void pan_protocol_set_global_fog_mode(SOCKET s, unsigned long mode) {
 ...
 * IMPLEMENTS SetGlobalFogProperties (286)
 */
void pan_protocol_set_global_fog_properties(SOCKET s, double radius,
                                            double density, double lin0,
                                            double lin1) {
  char *error =
      pan_net_set_global_fog_properties_TX(s, radius, density, lin0, lin1);
  if (!error)
    return;
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
 ...
 * IMPLEMENTS SetAtmosphereMode (287)
 */
void pan_protocol_set_atmosphere_mode(SOCKET s, unsigned long smode,
                                      unsigned long gmode,
                                      unsigned long amode) {
 ...
 * IMPLEMENTS SelectCamera (288)
 */
void pan_protocol_select_camera(SOCKET s, unsigned long cid) {
 ...
 * IMPLEMENTS BindLightToCamera (289)
 */
void pan_protocol_bind_light_to_camera(SOCKET s, unsigned long lid,
                                       unsigned long cid, unsigned char en) {
 ...
 * IMPLEMENTS ConfigureLightByDegrees (290)
 */
void pan_protocol_configure_light_by_degrees(SOCKET s, unsigned long lid,
                                             double r, double g, double b,
                                             double h, double e) {
 ...
 * IMPLEMENTS ConfigureLightByRadians (291)
 */
void pan_protocol_configure_light_by_radians(SOCKET s, unsigned long lid,
                                             double r, double g, double b,
                                             double h, double e) {
 ...
 * IMPLEMENTS SetLightPositionDirection (292)
 */
void pan_protocol_set_light_position_direction(SOCKET s, unsigned long lid,
                                               double ox, double oy, double oz,
                                               double dx, double dy,
                                               double dz) {
  char *error =
      pan_net_set_light_position_direction_TX(s, lid, ox, oy, oz, dx, dy, dz);
  if (!error)
    return;
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
 ...
 * IMPLEMENTS RenderToHoldBuffer (293)
 */
void pan_protocol_render_to_hold_buffer(SOCKET s, unsigned long cid,
                                        unsigned long bid) {
 ...
 * IMPLEMENTS DisplayHoldBuffer (294)
 */
void pan_protocol_display_hold_buffer(SOCKET s, unsigned long bid) {
 ...
 * IMPLEMENTS SetCornerCubesD (295)
 */
void pan_protocol_set_corner_cubes_d(SOCKET s, unsigned long n,
                                     unsigned long fmt, double *pcc) {
 ...
 * IMPLEMENTS SetProjectionMode (296)
 */
void pan_protocol_set_projection_mode(SOCKET s, unsigned long cid,
                                      unsigned long mode) {
 ...
 * IMPLEMENTS SetOrthoFieldOfView (297)
 */
void pan_protocol_set_ortho_field_of_view(SOCKET s, unsigned long cid,
                                          double width, double height) {
 ...
 * IMPLEMENTS SetLidarScan (298)
 */
void pan_protocol_set_lidar_scan(SOCKET s, unsigned long n, unsigned long fmt,
                                 double *pls // pointer to lidar scan samples
) {
 ...
 * IMPLEMENTS SetCameraMotion (299)
 */
void pan_protocol_set_camera_motion(
    SOCKET s, unsigned long cid, double vx, double vy,
    double vz,                       /* linear velocity */
    double rx, double ry, double rz, /* angular velocity */
    double ax, double ay, double az, /* linear acceleration */
  ...
  char *error =
      pan_net_set_camera_motion_TX(s, cid, vx, vy, vz, rx, ry, rz, ax, ay, az,
                                   sx, sy, sz, jx, jy, jz, tx, ty, tz);
  if (!error)
    return;
  (void)fprintf(stderr, "%s", error);
  (void)exit(1);
